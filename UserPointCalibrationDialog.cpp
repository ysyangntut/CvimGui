...
QImage UserPointCalibrationDialog::opencvMatToQImage(const cv::Mat &img, std::string filename)
{
    ...
    if (img.channels() == 3)
        qimg = QImage(this->img.data, this->img.cols, this->img.rows,
                      (int) this->img.step, QImage::Format_BGR888);
    else if (img.channels() == 1)
        qimg = QImage(this->img.data, this->img.cols, this->img.rows,
                      (int) this->img.step, QImage::Format_Grayscale8);
    else if (img.channels() == 4)  {
        QMessageBox msgBox;
        msgBox.setText("Warning: 4-channel image. Assuming ARGB32: " + QString::fromStdString(filename));
        msgBox.exec();
        qimg = QImage(this->img.data, this->img.cols, this->img.rows,
                      (int) this->img.step, QImage::Format_ARGB32);
    }
    else {
        QMessageBox msgBox;
...
//    }
    ...
    try {
        ...
        snprintf(buf, 1000, "Warning: Failed to get camera parameters "
                 "from ui widgets.\n");
        this->displayMessage(buf);
        return;
    }
...
void UserPointCalibrationDialog::on_pbUndistort_clicked()
{
    ...
    imgUndistortFile = QFileDialog::getSaveFileName
            (this, tr("Save Undistorted Image to File"), "./", tr("Images (*.png *.bmp *.jpg)"));
